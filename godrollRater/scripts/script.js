  ...
  if (code != null){
    ...
    for (var hashList in gunHashList){
      if (!(gunHashList[hashList] in apiCallRolls[membership_id]["roll_scores"])){
        ...
        apiCallRolls[membership_id]["roll_scores"][gunHashList[hashList]] = 0;
      }
    }
...
function addRow(parsedPerks, gunInfo, masterworkInfo, masterworkName, masterworkScore, rollRating, isFirstGun, rollScoreDisplay){
  ...
  if (totalRating >= 0.62 && totalRating < 0.81) {
    ...
    if (isFirstGun) {
      apiCallRolls[membership_id]["roll_scores"][gunInfo['Response']['hash']] = 1;
    }
  }
    ...
    if (isFirstGun) {
      apiCallRolls[membership_id]["roll_scores"][gunInfo['Response']['hash']] = 3;
    }
  }
    ...
    if (isFirstGun) {
      apiCallRolls[membership_id]["roll_scores"][gunInfo['Response']['hash']] = 5;
    }
  }
  else {
    cell.classList.add(colours[3])
    ...
    if (isFirstGun) {
      apiCallRolls[membership_id]["roll_scores"][gunInfo['Response']['hash']] = 0;
    }
  }
